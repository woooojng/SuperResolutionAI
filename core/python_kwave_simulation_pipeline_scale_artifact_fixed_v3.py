...
class TissuePropertiesManager:
    ...
    def create_tissue_maps(self, tumor_mask_3d: np.ndarray, grf: np.ndarray, 
        ...
        # Multi-scale 2-D fields produce more distinct patches.
        patch_raw_large = np.random.standard_normal((Nx_tot, Ny_tot))
        patch_raw_medium = np.random.standard_normal((Nx_tot, Ny_tot))
        patch_raw_small = np.random.standard_normal((Nx_tot, Ny_tot))

        patch_large = gaussian_filter(
            patch_raw_large, sigma=(26.0, 26.0), mode='reflect'
        )
        patch_medium = gaussian_filter(
            patch_raw_medium, sigma=(12.0, 12.0), mode='reflect'
        )
        patch_small = gaussian_filter(
            patch_raw_small, sigma=(5.0, 5.0), mode='reflect'
        )
        ...
        shared_fine = np.random.standard_normal(shape)
        sos_independent = np.random.standard_normal(shape)
        density_independent = np.random.standard_normal(shape)
...
class UltrasoundSimulator:
    ...
    def run_simulation(self, sample_name: str, input_data_path: str, output_path: str,
        ...
        # Generate GRF
        print(f"Generating GRF with sigma {config.grf_sigma}...")
        grf = self.grf_generator.create_gaussian_random_field(
            Nx_tot, Ny_tot, Nz_tot,
            config.grf_sigma, config.grf_kernel_size, config.coherence_level
        )
        ###
